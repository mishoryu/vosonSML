...
#' @param directIsolates direct top level comments to video creator.
...
#' @param verbose boolean, if \code{TRUE} prints processing messages.
...
#'
#' @export
CreateActorNetwork.youtube2 <- function(commentsData, writeToFile=FALSE, directIsolates=TRUE, verbose=FALSE) {
  ...
  # purely to avoid R CMD check notes, need to be vigilant these exist in data frames
  videoId <- authorChannelId.value <- authorDisplayName <- commentToAuthorChannelId <- NULL
  ...
  if (directIsolates) {
    ...
    for (row in 1:nrow(video_creators)) {
      ...
      video_author <- GetYoutubeVideoAuthor(video_creators[row, "videoId"], verbose)
  ...
  authorChannelId.value <- authorDisplayName <- NULL
  video_comment_actor_network_weighted <- CreateActorEdgeNetwork(commentsData, video_creators, video_comment_actors, directIsolates) %>%
                                          dplyr::group_by(authorChannelId.value, commentToAuthorChannelId) %>%
                                          dplyr::summarise(weight = dplyr::n()) %>% dplyr::ungroup()

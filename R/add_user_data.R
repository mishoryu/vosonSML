...
#' attributes.
#'
#' @note Only supports twitter actor networks at this time. Refer to \code{\link{add_user_data.actor.twitter}}.
#' 
...
#' 
#' @aliases add_user_data
#' @name vosonSML::add_user_data
#' @export
add_user_data <- function(net, data, ...) {
  # searches the class list of net for matching method
  UseMethod("add_user_data", net)
}

#' @noRd
#' @export
add_user_data.default <- function(net, ...) {
  stop("Unknown network type passed to add_user_data.", call. = FALSE) 
}

#' @noRd
#' @method add_user_data actor
#' @export
add_user_data.actor <- function(net, ...) {
  UseMethod("add_user_data.actor", net)
}

#' @noRd
#' @export
add_user_data.actor.default <- function(net, ...) {
  stop("Unknown social media type passed to add_user_data.", call. = FALSE)
}
...
#'   add_user_data(twitter_data, twitterAuth = readRDS("~/.twitter_auth"))
#' 
#' # network nodes, edges and downloaded user data
#' # actor_network$nodes
#' # actor_network$edges
#' # actor_network$users
#' }
...
#' 
#' @aliases add_user_data.actor.twitter
#' @name vosonSML::add_user_data.actor.twitter
#' @export
add_user_data.actor.twitter <- function(net, data, lookupUsers = TRUE, twitterAuth = NULL, verbose = TRUE, ...) {
  
  if (!requireNamespace("rtweet", quietly = TRUE)) {
    stop("Please install the rtweet package before calling add_user_data.", call. = FALSE)
  }

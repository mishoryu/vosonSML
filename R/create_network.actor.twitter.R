...
#' @param rm_edge_types Character vector. List of edge types to remove from network. Options are \code{"tweet"},
#' \code{"retweet"}, \code{"reply"} and \code{"quote"}. Default is \code{NULL}.
#' @param incl_mentions Logical. Create edges for users mentioned or tagged in tweets. Default is \code{TRUE}.
#' @param incl_rt_mentions Logical. Create edges for users mentioned or tagged in retweets. Default is \code{FALSE}.
...
#'   create_network("actor", rm_edge_types = c("retweet", "quote"), incl_mentions = FALSE)
#' 
...
#' 
#' @export
create_network.actor.twitter <-
  function(datasource,
           type,
           rm_edge_types = NULL,
           incl_mentions = TRUE,
           incl_rt_mentions = FALSE,
           verbose = TRUE,
           ...) {
    ...
    rm_edge_types <- rm_edge_types[trimws(tolower(rm_edge_types)) %in% types]
    
    edges <- edges %>% filter(!(.data$type %in% rm_edge_types))
    ...
    if (incl_mentions) {
      ...
      if (!("retweet" %in% rm_edge_types) && incl_rt_mentions) {
        retweet_mentions <-
    ...
    edges_rq <- edges_rq %>% filter(!(.data$type %in% rm_edge_types))
    ...
    nodes <- dplyr::bind_rows(
      dplyr::select(edges, id = "from", screen_name = "from_screen_name"),
      dplyr::select(edges, id = "to", screen_name = "to_screen_name")
    ) %>%
      dplyr::distinct() %>%

...
#' @examples
#' \dontrun{
#' # create a youtube actor network
#' actor_net <- youtube_data %>% create_network("actor")
#' 
...
#' 
#' @export
create_network.actor.youtube <- function(datasource, type, ...) {
  ...
  video_ids <-
    datasource %>% distinct(.data$VideoID) %>% dplyr::mutate(id = paste0("VIDEOID:", .data$VideoID)) %>%

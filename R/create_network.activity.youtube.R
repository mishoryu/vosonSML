...
#' activity_net <- youtube_data %>% create_network("activity")
#' 
...
#' 
#' @export
create_network.activity.youtube <-
  function(datasource, type, verbose = TRUE, ...) {
    ...
    for (row in 1:nrow(node_summary)) {
      ...
      if (type == "comment")
        df_stats %<>% network_stats("top-level comments", node_summary[row, "num"])
      else if (type == "reply-comment")
        df_stats %<>% network_stats("reply comments", node_summary[row, "num"])
      else if (type == "video")
        df_stats %<>% network_stats("videos", node_summary[row, "num"])
    }
    df_stats %<>% network_stats("nodes", nrow(df_nodes))
    df_stats %<>% network_stats("edges", nrow(df_relations))

...
#' twitter_data <- import_data("./data.csv", "twitter")
#' 
#' # import collected data from dataframe
#' twitter_data <- import_data(rtweet_data, "twitter")
#' }
#' 
#' @aliases import_data
#' @name vosonSML::import_data
#' @export
  ...
  if (!inherits(data, "data.frame")) {
    ...
    if (type == "csv") {
      if (!requireNamespace("readr", quietly = TRUE)) {
        stop("Please install the readr package before calling import_data.",
             call. = FALSE)
      }
    }
